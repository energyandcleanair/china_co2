...
translateSources <- function(x, lang=parent.frame()$lang) {
  if(lang=='ZH')
...
translateProvinces <- function(x, lang=parent.frame()$lang) {
...
translateFuels <- function(x, lang=parent.frame()$lang) {
  recode_factor(x,
                'Steam Coal'='动力煤',
                'Natural Gas'='天然气',
                'Crude Oil'='石油',
...
monthlab <- function(x, lang=parent.frame()$lang) {
  if(lang=='EN') x %<>% format.Date('%b')
  if(lang=='ZH') x %<>% month() %>% paste0('月')
  return(x)
}

yearlab <- function(x, lang=parent.frame()$lang) {
  if(is.Date(x) | is.POSIXt(x)) x %<>% year
  if(lang=='ZH') x %<>% paste0('年')
  x
}

convert_value <- function(x, original_unit, lang=parent.frame()$lang) {
...
unit_label <- function(original_unit, lang=parent.frame()$lang) {
  if(lang=='ZH') {
...
lang_theme <- function(lang=parent.frame()$lang) {
...
strsplit_lang <- function(x, width, lang=parent.frame()$lang) {

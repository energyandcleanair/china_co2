...
energy_mix %>%
  ...
  filter(year(date)>=2023, month(date) %in% 1:9) %>%
  ...
  write_csv(file.path(output_dir, '2024Q1-3 energy sudoku.csv'))

read_xlsx(file.path(output_dir, '2024Q1-3 energy sudoku.xlsx'), skip = 1) %>%

...
output_dir='outputs/2024Q2'
dir.create(output_dir)

last_month = ymd('2024-06-30')
...
d$Value1m[d$date==last_month & ind] <- d$Value1m[d$date==last_month_last_year & ind] * 1042.6/1038.9
...
source('project_workflows/2024Q2 NBS reported vs Lauri dataset.R')
...
d.quarter %>%
  filter(include_in_totals, grepl('NBS', name), date>='2012-01-01') %>%
...
d.quarter %>% filter(include_in_totals, grepl('predicted', name), year(date)>=2012) %>%
...
d.quarter %>%
  filter(include_in_totals, grepl('NBS', name), date>='2018-01-01',
...
d.quarter %>%
  filter(include_in_totals, grepl('predicted', name),
         month(date) %in% c(3,6,9,12)) %>%
...
d.changes %>% filter(prod != 'Total', date==last_month) %>%
  ...
  labs(title="Contributions to changes in emissions",
       subtitle="in Q2 2024, compared with 2023", y='Mt CO2 / year', x='') +
  coord_flip() +
  #x_at_zero() +

...
prod %<>% mutate(sector_coal=
                   case_when(#grepl('Thermal Power$', prod)~'Power Industry',
                     ...
                     grepl('Coke|Pig Iron', prod)~'Coking',
...
prod %<>% group_by(prod, month(date)) %>%
  mutate(yoy=Value1m/lag(Value1m)) %>%
  group_by(prod) %>% fill(yoy, .direction = 'down') %>%
  group_by(prod, month(date)) %>% mutate(Value1m=na.cover(Value1m, lag(Value1m)*yoy))
...
coaluse_pred %>% lapply('[[', 'data') %>% bind_rows() %>%
  ...
  #earlier: Chemical|Heating|Other|Coking
  mutate(coal_use_predicted = case_when((grepl('Chemical|Heating|Other', sector_coal) |
                                           grepl('Metallurgy', sector_coal) & energy_product=='Steam Coal')&
                                          !is.na(coal_use)~coal_use,
...
readwindEN(infile, c('var', 'sector'), read_vardata = T, zero_as_NA = T, skip=2) -> elec_cons_sector
...
dd %>%
  ...
  theme_crea() +
  scale_color_manual(values=colorRampPalette(crea_palettes$change)(length(unique(year(dd$date))))) +
  x_at_zero() +

...
d.quarter %>% filter(sector=='Total' | prod=='Coking Coal', prod!='Cement', grepl('predicted', name)) %>%
  mutate(broad_prod=case_when(grepl('Coal', prod)~'Coal', T~broad_prod),
         Mtce=CO2/case_when(broad_prod=='Coal'~85, broad_prod=='Oil'~72, broad_prod=='Gas'~55)/29.3*1e3) %>%
...
energy_mix %>% filter(year(date)==year(last_month)-1) %>%
  group_by(broad_prod) %>%
...
energy_mix %>%
  ...
  filter(year(date)>=2023, month(date) %in% 1:month(last_month)) %>%
  ...
  arrange(period, broad_prod) %T>% copy.xl() %>%
  write_csv(file.path(output_dir, '2024Q1-4 energy sudoku.csv'))

read_xlsx(file.path(output_dir, '2024Q1-4 energy sudoku.xlsx'), sheet='Q4', skip = 1) %>%

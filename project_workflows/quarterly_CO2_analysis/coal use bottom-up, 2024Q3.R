...
paste0('https://api.energyandcleanair.org/v1/',
       ...
       '&region_type=gadm1',
       #'&region_id=CN',
       '&region_iso2=CN') %>% read_csv -> dd

pop <- read_csv(get_data_file('population_by_province_2022.csv'))

dd %>% mutate(date = date %>% 'day<-'(days_in_month(.)),
...
#extrapolate reported coal use growth for chemicals, heating, other
coaluse_df %>%
  filter(name=='reported', sector_coal %in% c('Chemical', 'Other', 'Heating')) %>%
  group_by(sector_coal, month(date)) %>%
  mutate(yoy=value/lag(value)) %>%
  group_by(sector_coal) %>%
  fill(yoy, .direction='down') %>%
  group_by(sector_coal, month(date)) %>%
  mutate(value=na.cover(value, lag(value)*yoy)) ->
  coaluse_filled

coaluse_df %<>% anti_join(coaluse_filled %>% select(name, sector_coal)) %>%
  bind_rows(coaluse_filled %>% ungroup %>% select(-yoy))
...
coaluse_df %>% filter(sector_coal %in% c('Power Industry', 'Total'),
                      grepl('predict', name),
                      date<=last_month) %>%
...
coaluse_plot %>% filter(date>='2016-01-01', sector_coal == 'Power Industry') %>%
...
coaluse_plot %>%
  filter(date>='2016-01-01', date<=last_month, sector_coal %in% c('Power Industry', 'Total'), !is.na(value)) %>%
...
dd_monthly %>% ungroup %>% filter(year(date)>=2021) %>%
  ...
  theme_crea() + scale_color_manual(values=colorRampPalette(crea_palettes$change)(4)) + x_at_zero() +

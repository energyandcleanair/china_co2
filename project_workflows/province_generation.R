...
provdata_filled %>% filter(prov!='Tibet') %>%
  ...
  summarise(across(starts_with('Value'), ~.x[date==max(date)]/.x[date=='2020-12-31']-1)) %>%
  left_join(changes_total %>% select(prov, change_in_clean_share=share)) %>%
  ggplot(aes(Value_rollmean_12m, change_in_clean_share)) + geom_point() + geom_text_repel(aes(label=prov))
...
power_demand %>% group_by(prov) %>% summarise(across(Value, sum)) %>%

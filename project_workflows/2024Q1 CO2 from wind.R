...
output_dir='outputs/2024Q1'
dir.create(output_dir)

last_month = ymd('2024-03-31')

in_file = get_data_file("apparent consumption of fossils.xlsx")
getwindvars(in_file, skip=3) #%>% grep("YTD", ., value=T)
...
#update March natgas imports manually
ind=d$var=='Volume of Imports' & d$prod=='Natural Gas'
d$Value1m[d$date=='2024-03-31' & ind] <- d$Value1m[d$date=='2023-03-31' & ind] * 1.22
...
d$Value1m[d$date=='2014-01-31' & ind] <- d$Value1m[d$date=='2013-01-31'] * .9
...
coaluse_plot %>% filter(sector_coal=='Power Industry') %>% group_by(date) %>%
...
bottomup_adj %<>%
  group_by(name, prod, sector, month=month(date)) %>%
...
source('project_workflows/2024Q1 NBS reported vs Lauri dataset.R')

energy_mix_yoy_adj %>% select(broad_prod, nbs_adj=adj) %>%
  left_join(d.quarter %>% filter(grepl('predicted', name)), .) %>%
  mutate(nbs_adj=case_when(year(date)==2024&month(date) %in% 1:3 & !is.na(nbs_adj) ~nbs_adj, T~1),
         name='adjusted to NBS Q1 numbers') %>%
  bind_rows(d.quarter) ->
  d.quarter
...
d.quarter %>% filter(include_in_totals, grepl('NBS', name)) %>% ungroup %>%
...
d.quarter %>%
  filter(include_in_totals, grepl('NBS', name), date>='2015-01-01') %>%
...
d.quarter %>% filter(prod=='Total', grepl('NBS', name),
                     month(date) %in% (3*1:4),
                     year(date)>=2015) %>%
...
d.quarter %>% filter(include_in_totals, grepl('NBS', name), date>='2018-01-01') %>%
  ggplot(aes(date, YoY_change_1m)) + geom_col(aes(fill=prod)) +
  geom_point(data=d.quarter %>% filter(prod=='Total', grepl('NBS', name), date>='2018-01-01'), aes(col=prod)) +
...
d.changes %>%
  ggplot(aes(date, YoY_change_1m, fill=sector)) +
  facet_wrap(~prod) +
  geom_col() +
  scale_y_continuous(expand=expansion(c(.05,.05))) +
  theme_crea() +

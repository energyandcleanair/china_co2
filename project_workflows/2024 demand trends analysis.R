...
infile <- "inst/extdata/Electricity Consumption by sector 2025.xlsx"
infile %>% getwindvars(skip=2)
readwindEN(infile, c('var', 'sector', 'subsector'), read_vardata = T, zero_as_NA = T, skip=2) -> elec_cons_sector

elec_cons_sector %<>% filter(!grepl('\\(DC)', Name))

...
in_file = "apparent consumption of fossils 2022.xlsx"
...
readwindEN(in_file, c('var', 'prod', 'area', 'sector'), columnFilter = "Consump.*Steam", read_vardata = T) %>%
  mutate(type='M') -> d2
readwindEN(in_file, c('var', 'prod'), columnFilter = "Exports.*Oil", read_vardata = T) %>%
  mutate(type='M') -> d3
...
d %<>% ddply(.(var, prod, sector, type), unYoY) %>% filter(!YoY) %>% 
  ddply(.(var, prod, sector, type), unYTD, .progress='text')
...
d %<>% filter(grepl('Petroleum Processing|Finished Oil Products', prod)) %>% 
  mutate(prod='Oil Products') %>% 
  group_by(prod, date) %>% summarise_at('Value1m', sum) %>%
  mutate(var='Apparent Consumption') %>% bind_rows(d, .)

d %>% filter(prod=='Natural Gas', var=='Apparent Consumption') -> ngcons
d$Value1m[d$prod=='Natural Gas' & d$var=='Apparent Consumption' & d$date=='2021-12-31'] <-
  ngcons$Value1m[ngcons$date=='2020-12-31']*
  ngcons$Value1m[ngcons$date=='2021-11-30']/ngcons$Value1m[ngcons$date=='2020-11-30']

d$Value1m[d$prod=='Natural Gas' & d$var=='Apparent Consumption' & d$date=='2022-06-30'] <-
  ngcons$Value1m[ngcons$date=='2021-06-30']*
  ngcons$Value1m[ngcons$date=='2022-05-31']/ngcons$Value1m[ngcons$date=='2021-05-31']
...
if(F) {
  "Year|Coal|Crude Oil|Natural Gas|end_month
2020|.006|.033|.072|12
2021|.046|.041|.125|12
2022|.000|NA|.042|3" %>% textConnection %>% read.table(sep='|', header=T) %>%
    gather(prod, yoy, -Year, -end_month) -> adj
  
  d$sector %<>% na.cover('Total')
  for(i in 1:nrow(adj)) {
    if(!is.na(adj$yoy[i])) {
      prod.ind = grepl(adj$prod[i], d$prod) & grepl('Consumption', d$var) & month(d$date)<=adj$end_month[i]
      sum.ind = (d$prod!='Steam Coal' | d$sector == 'Total')
      ind1 = year(d$date)==adj$Year[i] & prod.ind
      ind0 = year(d$date)==adj$Year[i]-1 & prod.ind
      val1 = sum(d$Value1m[ind1 & sum.ind])
      val1_adj = sum(d$Value1m[ind0 & sum.ind]) * (1+adj$yoy[i])
      scaling = val1_adj / val1
    }
    print(paste(adj$prod[i], adj$Year[i], 'adjusting', sum(ind1), 'data points by', scaling))
    d$Value1m[ind1] %<>% multiply_by(scaling)
  }
  
  d %>% filter(date>'2019-01-01') %>%
    ggplot(aes(date, Value1m, col=paste(var, sector))) +
    geom_line() + facet_wrap(~paste(prod), scales='free_y')
  
}

d$Value1m[year(d$date)==2021 & month(d$date) %in% 7:9] %<>% multiply_by(.99)
...
CO2.factor <- c(Cement = 858.200/233035.7, #Cement = 769.300/231624.90 #2020 data https://zenodo.org/record/5126601
                ...
                Gasoline = 43.0*19.88*11/3/1e3/100,
                'Diesel Oil' = 42.8*20.0727*11/3/1e3/100,
                'Fuel Oil' = 40.5*21.4909*11/3/1e3/100,
                'Natural Gas' = 36.0*55.04/100/100,
                'Oil Products' = NA) %>%
  data.frame(cat=names(.), ef=.)

d %>% filter(grepl('Diesel|Fuel Oil|Gasoline', prod), grepl('Consumption', var), year(date)==2021, month(date)==11) %>% 
...
d$cat[d$prod == 'Kerosene'] <- 'Oil'
d$cat[d$prod == 'Petroleum Coke'] <- 'Oil'
...
d.adj %<>% ddply(.(var, prod, sector, type), roll12m, months=12, outcol='CO2_12m', incol='CO2') %>%
  ddply(.(var, prod, sector, type), roll12m, months=3, outcol='CO2_3m', incol='CO2')


#quarterly
d.adj %>% filter(date>'2010-01-01',date<='2022-06-30',
...
d.quarter %<>% filter(include_in_totals) %>% 
  group_by(date) %>% summarise(across(c(starts_with('CO2')), sum)) %>% 
  mutate(include_in_totals=F, prod='Total') %>% 
  bind_rows(d.quarter)

d.quarter %<>% group_by(prod, sector) %>%
  mutate(YoY_change_3m = CO2_3m - lag(CO2_3m, 4),
         YoY_3m = CO2_3m / lag(CO2_3m, 4) - 1)

output_dir=boxpath('CO2data/China/2022Q2 analysis')
...
prodcols = c(crea_palettes$dramatic[c(6,5,3,2)], 'gray30') #
names(prodcols) = unique(d.quarter$prod)[-1]
prodcols <- prodcols[c(5, 1, 2, 3, 4)]
...
d.quarter %>% filter(include_in_totals | prod=='Total') %>% 
...
d.quarter %>% filter(include_in_totals) %>%
  ...
  geom_point(data=d.quarter %>% filter(prod=='Total'), aes(col=prod)) +
  scale_x_date(expand=c(0,0), limits = c(ymd('2012-01-31'), NA)) +
  theme_crea() +
  scale_fill_manual(values=prodcols, name='Product') +
  ...
  theme(plot.title = element_text(size=rel(1.8)))
ggsave(file.path(output_dir, 'CO2 quarterly change by fuel.png'), 
       width=8, height=6)

d.quarter %>% filter(prod=='Total') %>%
  ggplot(aes(date, YoY_change_3m, fill=YoY_change_3m<0)) + geom_col() +
  scale_x_date(expand=c(0,0), limits = c(ymd('2012-01-01'), NA)) +
  theme_crea() +
  labs(title="China's CO2 emissions from energy and cement",
       subtitle="quarterly change, year-to-year", y='Mt CO2 / quarter', x='') +
  theme(plot.title = element_text(size=rel(1.8))) +
  scale_fill_crea_d('change', col.index = c(7, 1), guide='none')
ggsave(file.path(output_dir, 'CO2 quarterly change.png'), 
       width=8, height=6)
...
d.quarter %>% filter(include_in_totals) %>%
  ggplot(aes(date, CO2_3m*3, fill=prod)) +
  geom_area() +
  scale_x_date(expand=c(0,0)) +
  scale_y_continuous(expand=expansion(c(0,.05))) +
  theme_crea() +
...
d.quarter %>% filter(lubridate::quarter(date, with_year = T)==2022.2, 
...
d.changes %>% filter(prod != 'Total') %>% 
  ...
  labs(title="Contributions to changes in emissions",
       subtitle="in the second quarter of 2022, compared with 2021", y='Mt CO2 / year', x='') +
  coord_flip() + 
  scale_x_discrete(limits=rev)
ggsave(file.path(output_dir, 'Contributions to emissions growth.png'), 
       width=8, height=6)
...
in_file='Power Consumption By Industry M.xlsx'
getwindvars(in_file)
readwindEN(in_file,
           c('var', 'sector', 'subsector'),
           columnFilter=':') %>%
  mutate(type='M') -> elec
...
elec %>% ddply(.(var, sector, subsector), roll12m, months=3, incol='Value') %>% 
  ddply(.(var, sector, subsector), get.yoy, col='Value3m') %>% 
